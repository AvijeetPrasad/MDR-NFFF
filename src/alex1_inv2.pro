...
;___________________________________________________________________
;
      PRO ALEX1_inv2,ITAPERx,ITAPERy,ALPHA,Z,BZp,BXP,BYP,xoy,OBLIQUE,$
...
;                MATRIX VERSION  -  inversed direction
...
;                      (Note: Use even arrays or add appropriate border)
      IF OBLIQUE eq 1 then begin ; BZO is either bxp, or byp
      ...
      if xoy eq -1 then bzo=bxp else bzo=byp
         Siz=SIZE(BZO)
   ...
   ;  BORDER APPLIED (to minimized effect of periodic boundary conditions/side walls)
        ...
        ENDIF
   ;
      ;BXP=fltarr(NUMx,NUMy)
      ;BYP=fltarr(NUMx,NUMy)
      BZP=dblarr(NUMx,NUMy)        ; output
     ;
...
;     SMOOTH EDGE TO ZERO VIA COSINE FUNCTION
      ;BZ=DCOMPLEX(BZO,0.)
      bx=dcomplex(bxp,0.)
      by=dcomplex(byp,0.)  ; now for border = 0 only
...
;     FAST FOURIER TRANSFORM OF BZ(0)
     ...
     Bx=FFT(Bx,-1,/Double,/Overwrite)
     By=FFT(By,-1,/Double,/Overwrite);
;
...
;
;         GX=Make_array(Value=Complex(0.,0.),Lx,Ly)  ; TRANSFORMED GREEN'S FUNCTIONS
;         GY=Make_array(Value=Complex(0.,0.),Lx,Ly)
;         GZ=Make_array(Value=Complex(1.,0.),Lx,Ly)
;
            Q=SQRT(U^2+V^2)
         ...
         Bz=(dcomplex(0.,1.)*U*bx+dcomplex(0.,1.)*V*by)/Q
...
;              FKs=FK
;              GZ(small)=CoMPLeX(EXP(-FKs*Z),0.)
;              GY(small)=CoMPLeX(-(V(small)*FKs+U(small)*ALPHA)/PQ2(small),0.)
;              GY(small)=GY(small)*GZ(small)
;              GY(small)=GY(small)*Complex(0.,1.)  ; multibly by complex-i
;              GX(small)=CoMPLeX(-(U(small)*FKs-V(small)*ALPHA)/PQ2(small),0.)
;              GX(small)=GX(small)*GZ(small)
;              GX(small)=GX(small)*Complex(0.,1.)  ; multibly by complex-i
;             Endif
;;
...
;              ;C=0.001
;              C=0.0
;              GX(large)=CoMPLeX(-((UG*SZ-VA*CZ)-C*(UG*CZ+VA*SZ))/PQ2(large),0.)
;              GX(large)=GX(large)*Complex(0.,1.)
;              GY(large)=CoMPLeX(-((VG*SZ+UA*CZ)-C*(VG*CZ-UA*SZ))/PQ2(large),0.)
;              GY(large)=GY(large)*Complex(0.,1.)
;              GZ(large)=CoMPLeX(CZ+C*SZ,0.)
...
;;            ENDIF
;;
;        zero_u=WHERE( abs(GX) eq 0.)
;         zero_v=where( abs(GY) eq 0.)
;        GX(zero_u)=Complex(1.,0.)
;         GY(zero_v)=Complex(1.,0.)
;         ;GZ(zero_index)=Complex(1.,0.)
;
;         if xoy eq -1 then begin BZ=Bxp/GX & BZ(zero_u)=Byp(zero_u)/GY(zero_u)
;         endif else begin BZ=Byp/GY & BZ(zero_v)=Bxp(zero_v)/GX(zero_v)    ; RHS Bz= Transform of Bx, or By (Z=0)
;        endelse
;
;     TRANSFORM INVERSE
;
...
;
      ...
      ;BZO=BZO_save
      ;BXP=REFORM(FLOAT(BX(Border:(Border+Nxsav-1),Border:(Border+Nysav-1)) ),Nxsav,Nysav)
      ;BYP=REFORM(FLOAT(BY(Border:(Border+Nxsav-1),Border:(Border+Nysav-1)) ),Nxsav,Nysav)
      BZP=REFORM(double(BZp(Border:(Border+Nxsav-1),Border:(Border+Nysav-1)) ),Nxsav,Nysav)
      ENDIF
;
      RETURN
      END
