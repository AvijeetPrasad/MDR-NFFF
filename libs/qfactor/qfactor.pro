pro qfactor, bx, by, bz, xreg = xreg, yreg = yreg, zreg = zreg, csflag = csFlag, $
  ...
  ;  the efficiency of ifort (version 2021.4.0) is slightly (~1.40 times) faster
  ;  than gfortran (version 9.3.0)
  ;  please specify the path of qfactor.x at the line of "spawn, 'qfactor.x' " in
  ;  this file, or move qfactor.x to the $PATH (e.g. /usr/local/bin/) of system
  ;
  ;
  ; INPUTS
  ...
  ;
  ; OPTIONAL INPUTS
  ; xreg, yreg, zreg: pixel coordinates of the field region, in arrays of
  ;                   two elements default is to include all available pixels
  ;  --- If (xreg[0] ne xreg[1]) AND (yreg[0] ne yreg[1]) AND (zreg[0] ne zreg[1])
  ;      AND NOT csFlag calculate Q in a box volume invoke vFlag
  ;  --- If (xreg[0] eq xreg[1]) OR (yreg[0] eq yreg[1]) or (zreg[0] eq zreg[1])
  ;  calculate Q in a cross section perpendicular to X or Y or Z axis invoke cFlag
  ;  --- If zreg=[0, 0], calculate Q at the photosphere invoke z0Flag
  ;  --- If csFlag is set, see below invoke cFlag
  ;
  ...
  ;
  ; factor: to bloat up the original resolution, i.e. grid spacing = 1/factor;
  ;         default is 4
  ;
  ...
  ;
  ; scottFlag: calculate Q and Q_perp by the method of Scott_2017_ApJ_848_117;
  ;            default is 0B (method 3 of Pariat_2012_A&A_541_A78)
  ;       memory occupation in qfactor.x:
  ; 			if scottFlag=0B, 3D magnetic field + some 2D arrays
  ; 			if scottFlag=1B, 3D magnetic field + some 2D arrays + grad_unit_vec_B
  ;       (3 times as the 3D magnetic field)
  ;
  ; maxsteps: maxium steps for stracing a field line at one direction;
  ;           default is 40*(nx+ny+nz)/step
  ; twistFlag: to calculate twist number Tw; see Liu et al. (2016, ApJ);
  ;          default is 0
  ...
  ;
  ; tmpB: use temporary() to Bx, By, Bz to reduce the memory occupation of
  ;       3D magnetic field in IDL after writing b3d.bin to tmp_dir; default is 0B
  ; RAMtmp: use RAM (/dev/shm/tmp/) to speed up the data transmission;
  ;         default is 0B, if invoked,
  ;         please run only one task of qfactor.pro simultaneously on one machine
  ;
  ;
  ; OUTPUTS:
  ...
  ;
  ; slogq/slogq_perp: sign(Bz) x log_10(Q/Q_\perp)
  ;             if q = NaN, then slogq = 0.0. Calculated with all the field lines
  ; slogq_orig/slogq_perp_orig: only include field lines with both footpoints on
  ;             the photoshere
  ;
  ; twist/twist3d: twist number = \int \mu_0 J \cdot B /(4\pi) B^2 ds
  ;
  ; rboundary/rsboundary/reboundary: (s:start point, e:end point)
  ; nature of the ends of field lines,
  ; see 'subroutine vp_rboundary' in trace_bline.f90
  ;                0 - inside
  ;                1 - end at zmin
  ;                2 - end at zmax
  ;                3 - end at ymin
  ...
  ;   Jul  2,2014 R. Liu, introduce nchunks > nbridges to utilize
  ;               idle child processes
  ;   Apr 21,2015 R. Liu and J. Chen, deal with field lines pass through the
  ;               boundary other than bottom
  ...
  ;   Oct 29,2015 J. Chen, deal with field lines touching the cut plane: use
  ;               the plane quasi-perp to the field line
  ;   Nov 1, 2015 J. Chen,
  ...
  ;		so if rboundary3d[i, j, k] eq 9B, both two mapping surfaces of
  ;   q3d[i, j, k] are photosphere;
  ;   Jun 22,2016 J. Chen, add the map of field line length
  ;   Oct 30,2017 J. Chen, add the map of Bnr=abs(Bn_local/Bn_target) at the
  ;               photosphere; correct the output of Q: Modify -1,0 to NaN
  ;   Aug 28,2018 J. Chen, supplement Q at maginal points
  ;   May  1,2021 Peijing Zhang and J. Chen, trace field line with RKF45; RK4 is
  ;               remainded, modify classic RK4 to the 3/8-rule RK4
  ;   May  1,2021 J. Chen, adapted to gfortran compiler
  ;   Jun  1,2021 J. Chen, supplement Q at the point where surronding 4 points
  ;                   have different mapping planes; forcibly convert the
  ;                   input Bx, By, Bz to float arrays in IDL (Real(4) in Fortran)
  ;   Jun  5,2021 J. Chen, deal part of the points where surronding 4 points have
  ;             different mapping planes with the method of Scott_2017_ApJ_848_117
  ;   Jun 10,2021 J. Chen, provide a keyword of maxsteps for the case that has
  ;                  extremely long field lines
  ;   Jun 11,2021 J. Chen, add the coordinates of mapping points to '*.sav' data
  ;   Jun 13,2021 J. Chen, mark 0 for inside and 7 for others in
  ;               'subroutine vp_rboundary'
  ;   Jul  5,2021 J. Chen, switch the order of indexes of Bfield in
  ;               trace_bline.f90 for a better efficiency
  ;   Jul  9,2021 J. Chen, provide the option with the method of
  ;               Scott_2017_ApJ_848_117, and q_perp as an output
  ...
  ;   Jan 26,2022 J. Chen, fix a bug of RKF45 in case the difference between
  ;               RK4 and RK5 is 0
  ;   Jan 30,2022 J. Chen, remove the color table of doppler_color_mix,
  ;               due to the poor recognizability of green-white-yellow
  ...
  ; 	(6) cut_str can be a format of '(i0)', '(f0.1)' or '(f0.2)',
  ;       according to the value of input
  ...
  ;   May 19,2022 J. Chen, check the existence of nulls on grids;
  ;               add a keyword of RAMtmp
  ;
  ...
  if sbx[0] ne 3 or sby[0] ne 3 or sbz[0] ne 3 then $
    message, 'Bx, By and Bz must be 3D arrays!'
  if sbx[1] ne sby[1] or sbx[1] ne sbz[1] or $
    sbx[2] ne sby[2] or sbx[2] ne sbz[2] or $
    sbx[3] ne sby[3] or sbx[3] ne sbz[3] then $
    message, 'Bx, By and Bz must have the same dimensions!'
  nx = sbz[1]
  ny = sbz[2]
  nz = sbz[3]

  ; check the existence of nulls on grids,
  ; to avoid "segmentation fault occurred" in qfactor.x
  ...
  if csFlag and (zreg[0] eq zreg[1]) then $
    message, 'Something is wrong with the cross section .......'
  ...
  if (~no_preview) then begin
    ...
    else plots, [xreg[0], xreg[1], xreg[1], xreg[0], xreg[0]], [yreg[0], yreg[0], $
      yreg[1], yreg[1], yreg[0]], /dev
  ...
  if z0Flag then begin
    ...
    if (~no_preview) then begin
      ...
      ss = where(Bnr eq 0.0)
      Bnr_tmp = Bnr
      if (ss[0] ne -1) then Bnr_tmp[ss] = 1.
      ; to avoid "Program caused arithmetic error: Floating divide by 0"
    ...
    ; save results
    if scottFlag then begin
      if twistFlag then save, filename = file_sav, slogq, slogq_orig, q, length, $
        Bnr, rboundary, xreg, yreg, zreg, factor, $
        rF, q_perp, slogq_perp, slogq_perp_orig, twist $
      else save, filename = file_sav, slogq, slogq_orig, q, length, Bnr, rboundary, $
        xreg, yreg, zreg, factor, rF, q_perp, slogq_perp, slogq_perp_orig
    endif else begin
      if twistFlag then save, filename = file_sav, slogq, slogq_orig, q, length, $
        Bnr, rboundary, xreg, yreg, zreg, factor, rF, twist $
      else save, filename = file_sav, slogq, slogq_orig, q, length, Bnr, rboundary, $
        xreg, yreg, zreg, factor, rF
    endelse
  endif
  ...
  if cFlag then begin
    ...
    ; save results
    if scottFlag then begin
      if twistFlag then save, filename = file_sav, qcs, qcs_orig, length, $
        rsboundary, reboundary, xreg, yreg, zreg, factor, csFlag, rsF, reF, $
        q_perp, q_perp_orig, twist $
      else save, filename = file_sav, qcs, qcs_orig, length, rsboundary, reboundary, $
        xreg, yreg, zreg, factor, csFlag, rsF, reF, q_perp, q_perp_orig
    endif else begin
      if twistFlag then save, filename = file_sav, qcs, qcs_orig, length, rsboundary, $
        reboundary, xreg, yreg, zreg, factor, csFlag, rsF, reF, twist $
      else save, filename = file_sav, qcs, qcs_orig, length, rsboundary, reboundary, $
        xreg, yreg, zreg, factor, csFlag, rsF, reF
    endelse
  endif
  ...
  if vFlag then begin
    ...
    ; so if rboundary3d[i, j, k] eq 9B, both two mapping surfaces of q3d[i, j, k]
    ; are photosphere
    ...
    ; save results
    if scottFlag then begin
      if twistFlag then save, filename = file_sav, q3d, rboundary3d, xreg, yreg, $
        zreg, factor, q_perp3d, twist3d $
